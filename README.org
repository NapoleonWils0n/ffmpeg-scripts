...
scene-detect takes a video file and a threshold for the scene detection
from 0.1 to 0.9 and the -f seconds option to output the cut points in seconds
by default the script outputs the cut points in sexagesimal format,
hours minutes seconds, 00:00:00

+ script usage

#+BEGIN_SRC sh
scene-detect -i input -t (0.1 - 0.9) -f seconds
#+END_SRC
