...
create a bin directory, download the scripts with git and copy the scripts to the the bin directory

+ [[#bin-directory][create a bin directory]]
+ [[#git-repository][clone the git repository]]
+ [[#copy-symlink][copy the scripts to the bin directory]]
  
install ffmpeg on linux, mac, freebsd and windows
